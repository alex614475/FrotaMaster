...
namespace FrotaMaster.Infrastructure.Repositories
{
    public class ManutencaoRepository : IManutencaoRepository
    {
        ...
        public async Task AddAsync(Manutencao manutencao) => await _context.Manutencoes.AddAsync(manutencao);
        public async Task DeleteAsync(int id)
        {
            var manutencao = await _context.Manutencoes.FindAsync(id);
            if (manutencao != null) _context.Manutencoes.Remove(manutencao);
        }
        public async Task<IEnumerable<Manutencao>> GetAllAsync() => await _context.Manutencoes.AsNoTracking().Include(m => m.Veiculo).ToListAsync();
        public async Task<Manutencao?> GetByIdAsync(int id) => await _context.Manutencoes.Include(m => m.Veiculo).FirstOrDefaultAsync(m => m.Id == id);
        public async Task<IEnumerable<Manutencao>> GetByVeiculoIdAsync(int veiculoId) => await _context.Manutencoes.AsNoTracking().Where(m => m.VeiculoId == veiculoId).ToListAsync();
