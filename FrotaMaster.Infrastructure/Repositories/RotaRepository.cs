...
namespace FrotaMaster.Infrastructure.Repositories
{
    public class RotaRepository : IRotaRepository
    {
        ...
        public async Task<IEnumerable<Rota>> GetAllAsync() => await _context.Rotas.AsNoTracking().Include(r => r.Veiculo).Include(r => r.Motorista).ToListAsync();
        public async Task<Rota?> GetByIdAsync(int id) => await _context.Rotas.Include(r => r.Veiculo).Include(r => r.Motorista).FirstOrDefaultAsync(r => r.Id == id);
        public async Task UpdateAsync(Rota rota)
        {
            var existing = await _context.Rotas.FindAsync(rota.Id);
            if (existing != null) _context.Entry(existing).CurrentValues.SetValues(rota);
        }
